...
module.exports = class ReconciliationService extends cds.ApplicationService {
  ...
  async init() {
    ...
    this.on('approvePosting', async (req) => {
      ...
      if (deltaPercent > 1000) {
        ...
        await INSERT.into('tank.reconciliation.AuditLogEntry').entries({
          ...
          message: 'M4.license_check: HIGH VARIANCE WARNING — ' + deltaPercent.toFixed(0) + '% variance approved by ' + ((req.user && (req.user.attr?.email || req.user.id)) || 'supervisor') + '. Verify data quality before posting.',
          timestamp: now2, actor: (req.user && (req.user.attr?.email || req.user.id)) || 'supervisor'
        });
      }

      const now       = new Date().toISOString();
      const decidedBy = (req.user && (req.user.attr?.email || req.user.id)) || 'supervisor';
    ...
    this.on('retriggerDataCollection', async (req) => {
      ...
      const now   = new Date().toISOString();
      const actor = (req.user && (req.user.attr?.email || req.user.id)) || 'system';
...
<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:urn="urn:sap-com:document:sap:rfc:functions">
  <soapenv:Header/><soapenv:Body><urn:BAPI_TRANSACTION_ROLLBACK/></soapenv:Body>
          ...
          // Extract nomination number — handles namespace prefixes e.g. <urn:NOMINATIONNUMBER_SAP>
          const _extractNomNum = (body) => {
            const patterns = [
              /<(?:\w+:)?NOMINATIONNUMBER_SAP[^>]*>([^<]+)<\/(?:\w+:)?NOMINATIONNUMBER_SAP>/i,
              /<(?:\w+:)?NOMINATIONNUMBER_EXT[^>]*>([^<]+)<\/(?:\w+:)?NOMINATIONNUMBER_EXT>/i,
              /<(?:\w+:)?NOMINATIONNUMBER(?!_)[^>]*>([^<]+)<\/(?:\w+:)?NOMINATIONNUMBER>/i,
              /<(?:\w+:)?NOMINATION_NUMBER[^>]*>([^<]+)<\/(?:\w+:)?NOMINATION_NUMBER>/i,
            ];
