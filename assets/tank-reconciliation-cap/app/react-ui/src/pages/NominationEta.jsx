  ...
  async function submitCreateNomination() {
    ...
    try {
      ...
      if (result?.success) {
        ...
        setCreateMsg({ ok: true, text: `✅ Nomination created! Nom Number: ${nomNumber || '–'} | Nom Key: ${nomKey}` });
        setMessages(prev => [...prev, {
          role: 'assistant',
          text: [
            `✅ **Nomination Created Successfully**`,
            ``,
            `- **Nomination Number:** ${nomNumber || '–'} _(user defined)_`,
            `- **Nom Key:** ${nomKey} _(auto-generated by SAP)_`,
            ...
            itemLines,
            ``,
            `Would you like me to propose an ETA for nomination **${nomNumber || nomKey}**?`,
          ].join('\n'),
        }]);
          ...
          <div style={{
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem', marginBottom: '1rem' }}>
              <div style={{ gridColumn: '1 / -1' }}>
                <Label>Nomination Number (user defined)</Label>
                <Input style={{ width: '100%' }} value={form.Nominationnumber || ''}
                  onInput={e => setField('Nominationnumber', e.target.value)}
                  placeholder="Enter any reference number or name (e.g. NOM-2026-001)" />
              </div>
              <div>
